...
func completeWorkspaceConfig(wsConfig *cosmov1alpha1.Config, unst []unstructured.Unstructured) error {
	...
	if len(ingRoutes) > 0 {
		// complete ingress name
		if wsConfig.IngressRouteName == "" {
			if len(ingRoutes) == 1 {
				wsConfig.IngressRouteName = ingRoutes[0].GetName()
		...
		for _, v := range ingRoutes {
			if wsConfig.IngressRouteName == v.GetName() {
		...
		if !validIngRoute {
			return fmt.Errorf("ingressRoute '%s' is not found", wsConfig.IngressRouteName)
		}
	}
	return nil
}
