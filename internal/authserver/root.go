...
`,
		Version:           "v0.7.0 cosmo-workspace 2023",
	...
	rootCmd.PersistentFlags().StringVar(&o.RedirectUrl, "redirect-url", "", "Redirect URL")
	...
	rootCmd.PersistentFlags().IntVar(&o.ServerPort, "port", 8450, "Port for auth server")
...
func (o *options) Validate(cmd *cobra.Command, args []string) error {
	...
	if o.RedirectUrl == "" {
		return fmt.Errorf("%s is required", "redirect-url")
	}
	if !o.Insecure {
		if o.TLSCertPath == "" {
...
func (o *options) RunE(cmd *cobra.Command, args []string) error {
	...
	// Start server
	setupLog.Info("Start authserver")
	if err := mgr.Start(ctx); err != nil {
		setupLog.Error(err, "problem running authserver")
		os.Exit(1)
	}
...
func printVersion(cmd *cobra.Command, o *options) {
	fmt.Fprintf(cmd.OutOrStdout(), "authserver version %s\n", cmd.Version)
}
